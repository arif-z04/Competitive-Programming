...
template<typename T> using pq_ = priority_queue<T, vector<T>, greater<T>>;
...
// C11 has __gcd but C20 has gcd, make sure to remove it
#define gcd               __gcd;
...
i64 NcR(i64 n, i64 r){i64 x = 1ll, y = 1ll;if(n - r < r) r = n - r; while(r){x *= n;y *= r; i64 cm = __gcd(x, y);x /= cm;y /= cm;--n;--r;}return x;}
i64 lcm(i64 a, i64 b){ if(a > b) swap(a, b); return (a * (b / __gcd(a, b)));}
...
void solve(){
    ...
    {
        ui64 x; cin >> x;
        ui64 dist = 2 * max(p, n - 1 - p);
        if(x <= dist) possible = false;
    }

    cout << (possible ? yms[0] : nms[0]) << ln;
}
